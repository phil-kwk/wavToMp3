...
namespace encoder {
    ...
    template<>
    const std::vector<uint8_t>
    LameEncoder<float32>::encodePCMSamples(
            const ChannelContainer<float32>& samples) {
        ...
        int ret = 0;
        if (numberChannels == 2) {
            ret = lame_encode_buffer_ieee_float(lgf,
                    (const float *)&samples.at(0)[0].data,
                    (const float *)&samples.at(1)[0].data,
                    numberOfSamples,
                    &mp3Buffer[0],
                    mp3Buffer.size());
        } else if (numberChannels == 1) {
    ...
    template<>
    const std::vector<uint8_t>
    LameEncoder<float64>::encodePCMSamples(
            const ChannelContainer<float64>& samples) {
        ...
        int ret = 0;
        if (numberChannels == 2) {
            ret = lame_encode_buffer_ieee_double(lgf,
                    (const double *)&samples.at(0)[0].data,
                    (const double *)&samples.at(1)[0].data,
                    numberOfSamples,
                    &mp3Buffer[0],
                    mp3Buffer.size());
        } else if (numberChannels == 1) {
